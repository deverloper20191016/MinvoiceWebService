                    ...
                    foreach (var invoice in invoices)
                    {
                        ...
                        else
                        {
                            if (dataResponse.ContainsKey("ok") && dataResponse.ContainsKey("data"))
                            {
                                ...
                                if (typeUpdate == 2)
                                {
                                    jObjectResult.Add($"OK_NEW;{dataResponse["data"]["trang_thai_hd"]}_{trangThaiKyNew}", $"{dataRequestObject.MauSo};{dataRequestObject.KyHieu}-{invoice.Master.Key}_{dataResponse["data"]["inv_invoiceNumber"]}");
